#!/usr/bin/env python3
"""
MARATHON STRATEGY SWEEP $500 TARGET — Runs for 1 HOUR testing millions of strategies.

Target: $500 per session (5× the original $100 target)
Bet ranges scaled up to reach $500 within 60 spins.
...
# ── OVERRIDE: $500 target instead of $100 ──
SESSION_TARGET = 500.0
...
def rand_cap():
    return rand_choice([8.0, 10.0, 12.0, 15.0, 20.0, 25.0, 30.0, 40.0, 50.0, 75.0, 100.0, 200.0])

cap_label = lambda c: f'${c:.0f}' if c < 200 else 'NoCap'


def random_v1():
    base = rand_float(0.5, 20.0, 0.5)
    inc = rand_float(0.5, 15.0, 0.5)
    dec = rand_float(0.25, 8.0, 0.25)
...
def random_c():
    base = rand_float(0.5, 15.0, 0.5)
    div = rand_float(0.5, 8.0, 0.25)
    step = rand_float(0.5, 15.0, 0.5)
...
def random_j():
    base = rand_float(1.0, 20.0, 0.5)
    mult = rand_float(1.25, 5.0, 0.25)
...
def random_k():
    ...
    seq = tuple(random.randint(1, 20) for _ in range(seq_len))
    base = rand_float(0.5, 15.0, 0.5)
    cap = rand_cap()
    if cap < base:
        cap = 200.0
...
def random_q():
    hb = rand_float(5.0, 50.0, 1.0)
    cb = rand_float(0.5, 10.0, 0.5)
...
def random_s():
    ...
    agg = rand_float(5.0, 40.0, 1.0)
    con = rand_float(0.5, 8.0, 0.5)
    pt = rand_choice([50.0, 100.0, 150.0, 200.0, 250.0, 300.0])
    at = rand_choice([-50.0, -100.0, -150.0, -200.0, -300.0, -400.0, -500.0])
    return {
...
def random_o():
    base = rand_float(1.0, 15.0, 0.5)
    step_up = rand_float(0.5, 10.0, 0.5)
    pl = random.randint(3, 20)
    cap = rand_cap()
    if cap < base:
        cap = 200.0
    return {
...
def main():
    ...
    if global_top_safe:
        print_table("TOP 30 — SAFEST (smallest drawdown, 40%+ hit rate)", global_top_safe, 30)
    ...
    if global_top_overall:
        ...
        print(f"")
        print(f"  Results:")
        print(f"    Target hit rate:    {wr['hit_rate']}% of sessions reach +${SESSION_TARGET:.0f}")
